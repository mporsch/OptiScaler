...
template <typename T> struct OutInPtrTraits;

// clang-format off: keep the template arguments readable
template <template <typename Smart_, typename Pointer_, typename...> class OutInPtr,
          typename Smart_,
          typename Pointer_,
          typename... Args>
...
template <class OutInPtr> OutInPtr&& IID_OUTPTR_ARGS_Helper(OutInPtr&& outInPtr)
{
...
{
    inline void operator()(IUnknown* ptr) const noexcept { ptr->Release(); }
};
...
template <typename T> using ComPtr = std::unique_ptr<T, Detail::IUnknownDeleter>;
